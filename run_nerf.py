...
from load_zaragoza256 import load_zaragoza256_data
...
def train():
    ...
    model = torch.load('./pretrainmodel/generated_oneside_dim5.pt')
    # 预训练模型
    ...
    with torch.no_grad():
        ...
        camera_grid_positions = full_data[L:,:].cpu().numpy()
        del full_data
    ...
    for i in trange(start, N_iters):
        ...
        for m in range(0, M, 1):
            ...
            for n in range(0, N, 1):
                ...
                if args.test_neglect_former_bins:
                    [input_points, I1, I2, I0] = spherical_sample_histgram(I, L, camera_grid_positions[:,m * M + n], args.num_sampling_points, args.test_accurate_sampling, volume_position, volume_size, c, deltaT)
                    ...
                    if args.use_encoding:
                        input_points_coded = encoding_batch_numpy(input_points, args.encoding_dim)
                        input_points_coded = torch.from_numpy(input_points_coded).float().to(device)
                        # del input_points
                    # print(time.time() - time0)
                    network_res = model(input_points_coded)
                    '''
                    network_res = torch.empty(input_points_coded.shape[0]).to(device)
                    # print(time.time() - time0)
                    for l in range(ceil(input_points_coded.shape[0] / train_batchsize)):
                        input_batch = input_points_coded[0 + l * train_batchsize :train_batchsize + l * train_batchsize,:]
                        network_res[0 + l * train_batchsize :train_batchsize + l * train_batchsize] = model(input_batch).view(-1)
                        del input_batch'''
                    ...
                    # print(time.time()-time0)
                    '''
                    input_points = spherical_sample_histgram(I, L, camera_grid_positions[:,m * M + n], args.num_sampling_points, args.test_accurate_sampling, volume_position, volume_size)
                    input_points = input_points.reshape([-1,3])
                    # print(time.time()-time0)
                    # show_samples(input_points, volume_position, volume_size, camera_grid_positions[:,m * M + n])
                    ...
                    network_res = torch.empty(input_points_coded.shape[0]).to(device)
                    for l in range(ceil(input_points_coded.shape[0] / test_batchsize)):
                    ...
                    index = torch.zeros(L - I + 1).to(device)
                    for l in range(L - I):
                    ...
                    with torch.no_grad():
                        nlos_histogram = nlos_data[I1:(I1 + I0), m, n]
                else:
                ...
                with torch.no_grad():
                    # nlos_histogram = nlos_histogram / 200
                    # nlos_histogram = nlos_histogram * 300000
                    # nlos_histogram = nlos_histogram * 1000
                    nlos_histogram = nlos_histogram * 16000
# 
                if (n % 32 == 0) | ((m < 1) & (n < 64)):
                    plt.plot(nlos_histogram.cpu(), alpha = 0.5, label = 'data')
                    plt.plot(pred_histgram.cpu().detach().numpy(), alpha = 0.5, label='predicted')
                    plt.legend(loc='upper right')
                    plt.savefig('./figure/' + str(i) + '_' + str(m) + '_' + str(n) + 'histogram')
                    plt.close()

                # plt.plot(nlos_histogram.cpu())
                # plt.savefig('./figure/' + str(i) + str(m) + str(n) + 'data_histogram')
                # plt.close()
                # plt.plot(pred_histgram.cpu().detach().numpy())
                # plt.savefig('./figure/' + str(i) + str(m) + str(n) + 'pred_histogram')
                # plt.close()
                ...
                loss2 = torch.rand([1])# loss2 = 1 * criterion2(network_res, torch.zeros(network_res.shape).to(device))
                loss = loss1 #+ loss2 # 注意：loss 函数内部已经集成了平均值处理
                loss.backward()
                optimizer.step()
                optimizer.zero_grad()
                ...
                print(i,'/',N_iters,'iter  ', m,'/',nlos_data.shape[1],'  ', n,'/',nlos_data.shape[2], '  histgram loss: ',loss1.item(), loss2.item() , 'time: ', dt)
