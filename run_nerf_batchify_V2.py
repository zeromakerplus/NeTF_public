...
def train():
    ...
    if args.dataset_type == 'nlos':
        ...
        # volume_position = 
        # volume_size
        # volume_position = np.array([0,-0.8,0])
        # volume_size = 1.6
    ...
    # model = torch.load('./model/10.27test_#50/epoch1m119.pt', map_location = 'cuda:0')
        ...
        # for m in range(0, M, 1):
            ...
            while n < N - 1:
                ...
                with torch.no_grad():
                    ...
                    if torch.min(average_loss) > 0:
                        average = torch.mean(average_loss)
                        std = torch.std(average_loss)
                        if loss > (average + 2 * std):
                            n -= 1
                ...
                print(i,'/',N_iters,'iter  ', m,'/',nlos_data.shape[1],'  ', n_print,'/',nlos_data.shape[2], '  histgram loss: ',loss.item(), 'time: ', dt)
